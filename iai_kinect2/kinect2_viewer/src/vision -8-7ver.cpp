...
ros::Publisher pose_arm_pub,pose_car_pub,target_pub;

geometry_msgs::Vector3 pose_arm,pose_car;

kinect2_viewer::visual_interface poseout;
			...
			for(int i=0;i<9;i++)
			 {
				 ...
				 poseout.pose.pose.position.z=object[i].state;
				 poseout.theta=object[i].pose.rz;
  			  ...
  			  RotatedRect rect=minAreaRect(contoursco[i]);//画最小外接矩形
						 if((rect.size.width/rect.size.height)>1 || (rect.size.width/rect.size.height)<1)
					  {
						...
						muc[i] = moments(contoursco[i], false);
						 ...
						 {	
								if((rect.size.width/rect.size.height)>3/2 || (rect.size.width/rect.size.height)<2/3)
								{
						 float angle_qu=-rect.angle;
