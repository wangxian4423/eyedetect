...
ros::Publisher pose_arm_pub,pose_car_pub,target_pub,best_pub;

geometry_msgs::Vector3 pose_arm,pose_car;


kinect2_viewer::visual poseout;
geometry_msgs::Twist bp;
...
struct Object
{
	int obj_id;//0,1,2,3,4,5,6,7  0是三棱柱，1是四棱柱，2是六棱柱，3是圆柱，4是三棱柱对应装配体，5是四棱柱对应装配体，6是六棱柱对应装配体，6是圆柱对应装配体
 ...
 cv::Mat deff_co ;
Rect myrect(89,87,655,332);
Rect rect1(89,87,218,332);
Rect rect2(307,87,218,332);
Rect rect3(525,87,218,332);
 Point3f affpoint;
 Point3f affpointout;
float fx = 534.6526;
float fy = 534.8408;
  ...
  void mydepthViewer() //自己写得深度图像检测
  {
   ...
   //  cv::Mat deff_de = cv::Mat(depth.rows, depth.cols, CV_8U);
      ...
      const int font = cv::FONT_HERSHEY_SIMPLEX;
	   ...
	   cv::Rect maskrect(89,87,655,332);
			...
			for(int i=0;i<8;i++)
			 {
				...
				if(object[i].state == 1) //stand
				{
					switch(i)
					{
						case 1:poseout.visual[i].pose.pose.position.z = cameraz - 0.060 ; break;
						case 2:poseout.visual[i].pose.pose.position.z = cameraz - 0.060 ; break;
						case 0:poseout.visual[i].pose.pose.position.z = cameraz - 0.077 ; break;
						case 3:poseout.visual[i].pose.pose.position.z = cameraz - 0.077 ; break;
						case 5:poseout.visual[i].pose.pose.position.z = cameraz - 0.080 ; break;
						case 4:poseout.visual[i].pose.pose.position.z = cameraz - 0.080 ; break;
						case 6:poseout.visual[i].pose.pose.position.z = cameraz - 0.080 ; break;
						case 7:poseout.visual[i].pose.pose.position.z = cameraz - 0.080 ; break;
				...
				if(object[i].state == -1)
				{
					switch(i)
					{
						case 0:poseout.visual[i].pose.pose.position.z = cameraz - 0.020 ; break;
						case 1:poseout.visual[i].pose.pose.position.z = cameraz - 0.030 ; break;
						case 2:poseout.visual[i].pose.pose.position.z = cameraz - 0.024 ; break;
						case 3:poseout.visual[i].pose.pose.position.z = cameraz - 0.030 ; break;
						case 4:poseout.visual[i].pose.pose.position.z = cameraz - 0.080 ; break;
						case 5:poseout.visual[i].pose.pose.position.z = cameraz - 0.080 ; break;
						case 6:poseout.visual[i].pose.pose.position.z = cameraz - 0.080 ; break;
						case 7:poseout.visual[i].pose.pose.position.z = cameraz - 0.080 ; break;
				 ...
				 poseout.visual[i].pose.pose.position.x=(object[i].pose.x-cx)*poseout.visual[i].pose.pose.position.z/fx;
				poseout.visual[i].pose.pose.position.y=(object[i].pose.y-cy)*poseout.visual[i].pose.pose.position.z/fy;
...
distortionCoefficients: !!opencv-matrix
 d = inputDepth[u, v]
            ...
            y = (u - cameraIntrinsics[1,2]) * z / cameraIntrinsics[1,1]*/
				 ...
				 poseout.visual[i].theta=(90-object[i].pose.rz)/180*3.14159;
				 poseout.visual[i].state=object[i].state;
			 ...
			 bp.linear.x=(affpointout.x-cx)*affpointout.z/fx;
			 bp.linear.y=(affpointout.y-cy)*affpointout.z/fy;
			 bp.linear.z=affpointout.z;
			 best_pub.publish(bp);
				...
				// const pcl::PointXYZRGBA& pt1 = cloud->points[mc[i].y * depth.cols + mc[i].x];
				 ...
				 //poseout.pose.pose.position.y=pt.y;
				  ...
				  /************************立四棱柱****************************/
					...
					/*************立圆柱************/
					if((contourArea(contours[i])>160)&&(contourArea(contours[i])<240))
					{	
						mu[i] = moments(contours[i], false);
						 mc[i] = Point2d(mu[i].m10 / mu[i].m00, mu[i].m01 / mu[i].m00);//求中心点坐标
...
/************debug模式结束,正常工作模式如下*******************/
							if(!debug)
								...
								/************************卧四棱柱****************************/
									if( (contourArea(contours2[i])>440) && (contourArea(contours2[i])<630) )
									{
									...
									/*********************卧圆柱************************/
									if( (contourArea(contours2[i])>180) && (contourArea(contours2[i])<330) )
									{
										RotatedRect rect=minAreaRect(contours2[i]);//画最小外接矩形
										...
										if(((rect.size.width/rect.size.height)>1.2 || (rect.size.width/rect.size.height)<0.83) &&(rect.size.width<39 && rect.size.height<39))
									{
										...
										mc[i] = Point2d(mu[i].m10 / mu[i].m00, mu[i].m01 / mu[i].m00);//求中心点坐标
										 ...
										 if(myrect.contains(mc[i]))
										 {
											 if(deff_de.at<uint8_t>(mc[i].y-20,mc[i].x)<50)
											 {
										float angle_qu=-rect.angle;
										circle(colorin, mc[i], 5, Scalar(255, 0, 255), -1, 8, 0);//在彩色图上画出中心点
										char tam1[100];
									...
									/*********************卧六棱柱************************/
									if( (contourArea(contours2[i])>110) && (contourArea(contours2[i])<400) )
									{
										...
										if(((rect.size.width/rect.size.height)>1.1 || (rect.size.width/rect.size.height)<0.38) && (rect.size.width>39 || rect.size.height>39))
									{
...
/*****************************检测4个装配件卧着时的坐标信息****************************/
				  /************************卧三棱柱****************************/
  			  if ((200< contourArea(contoursco[i])) && (contourArea(contoursco[i])<550))
  			 	 {
  				
				 // drawContours(colorin,contoursco,i,Scalar(200,100,0), 1, 8);
				  // cout<<"1"<<endl;
  			  RotatedRect rect=minAreaRect(contoursco[i]);//画最小外接矩形
						 if((rect.size.width/rect.size.height)>3 || (rect.size.width/rect.size.height)<1/3)
					  {
						...
						muc[i] = moments(contoursco[i], false);
						 ...
						 {	
								if(rect.size.width<20||rect.size.height<20)//(rect.size.width/rect.size.height)>3/2 || (rect.size.width/rect.size.height)<2/3
								{
						 float angle_qu=-rect.angle;
	...
	if(imgThresholded.at<uint8_t>(affpoint.y,affpoint.x) != 0)
	{
...
int main(int argc, char **argv)
{
 ...
 ros::NodeHandle np;
	ros::Timer timer2 = np.createTimer(ros::Duration(0.1),TimerCallback1);
        best_pub= np.advertise<geometry_msgs::Twist>("kinect/bestpoint", 1000);
