			...
			/*********检测4个大孔件的坐标信息*********/
					...
					if((contourArea(contours[i])>2000)&&(contourArea(contours[i])<2600))
					{
						 ...
						 if((0.8*rect.size.height<rect.size.width) && (rect.size.width<1.2*rect.size.height))//
					  {
						...
						switch(count_hole)
						{//先把孔件初略分类
						case 1:num_con[count_hole+2]=i;break;
						case 2:num_con[count_hole+2]=i;break;
						case 3:num_con[count_hole+2]=i;break;
...
/************debug模式结束,正常工作模式如下*******************/
							if(!debug)
								...
								/************************卧四棱柱****************************/
									...
									/*********************卧圆柱************************/
									if( (contourArea(contours2[i])>200) && (contourArea(contours2[i])<310) )
									{
										RotatedRect rect=minAreaRect(contours2[i]);//画最小外接矩形
										if(1)
									{
									...
									/*********************卧六棱柱************************/
									if( (contourArea(contours2[i])>200) && (contourArea(contours2[i])<330) )
									{
										RotatedRect rect=minAreaRect(contours2[i]);//画最小外接矩形
...
/**************彩色参数*********/
  ...
  GaussianBlur(imgThresholded, imgThresholded, Size(3, 3), 0.1, 0, BORDER_DEFAULT);//
	...
	//canny算子，边缘检测
  	  ...
  	  Canny(imgThresholded, canny_output1, 30, 30 * 3, 3);

  	  findContours(canny_output1, contours1, hierarchy1, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE);
     // drawContours(colorin, contours,i,Scalar(50), 1, 8);
	 imshow("cannycolor",canny_output1);
  	  //********************
  	  if (contours1.size() == 0)
  	  {
  	  ...
  	  else
  	  {
  		  ...
  		  for (i = 0; i < (int)contours1.size(); i++)
  		  {
  			   
  			 vector<Moments> mu(contours1.size());//霍夫向量矩阵
			 vector<Point2f> mc(contours1.size());//中心点坐标向量存储矩阵

				 
/*****************************检测4个装配件卧着时的坐标信息****************************/
				  /************************卧三棱柱****************************/
  			  if ((150< contourArea(contours1[i])) && (contourArea(contours1[i])<600))
  			 	 {
  				// cout<<"1"<<endl;
  			  RotatedRect rect=minAreaRect(contours[i]);//画最小外接矩形
						 if((rect.size.width/rect.size.height)>1 || (rect.size.width/rect.size.height)<1)
					  {
						...
						mu[i] = moments(contours[i], false);
						 ...
						 if(myrect.contains(mc[i]))
						 {
							if(1)//deff_de.at<uint8_t>(mc[i].y,mc[i].x)<10

						 {	
 cout<<"2"<<endl;
						 float angle_qu=-rect.angle;
						 circle(colorin, mc[i], 5, Scalar(100, 100, 100), -1, 8, 0);//在彩色图上画出中心点
					...
					char tam1[100];
						sprintf(tam1, "(triangular,pose=%d,%d,%d,an=%0.1f)",pose,(int)mc[i].x, (int)mc[i].y,angle_qu);			
						putText(colorin, tam1, Point(mc[i].x, mc[i].y), FONT_HERSHEY_SIMPLEX, 0.4, cvScalar(0, 0,0), 1);}
					  }
					  }
  		  			}
