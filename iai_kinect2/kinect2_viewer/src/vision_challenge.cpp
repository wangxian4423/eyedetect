			...
			for(int i=0;i<9;i++)
			 {
				...
				if(object[i].state == 1) //stand
				{
					switch(i)
					{
						...
						case 2:poseout.obj[i].pose.pose.position.z = cameraz - 0.060 ; break;
						case 0:poseout.obj[i].pose.pose.position.z = cameraz - 0.077 ; break;
						case 3:poseout.obj[i].pose.pose.position.z = cameraz - 0.077 ; break;
			...
			imshow("tam1",tmp1);
			  ...
			  //对孔件进行重新排序
				  ...
				  int ih,jh,k;
						...
						for(ih=4;ih<9-jh;ih++)
							{
								if( mc[num_con[ih]].y > mc[num_con[ih+1]].y )
								{
									k=num_con[ih];
									num_con[ih]=num_con[ih+1];
									num_con[ih+1]=k;
								}
							}
						}
						 circle(colorin, mc[num_con[0]], 5, Scalar(0, 0,50), -1, 8, 0);
						 circle(colorin, mc[num_con[1]], 5, Scalar(0, 0,100), -1, 8, 0);
						 circle(colorin, mc[num_con[2]], 5, Scalar(0, 0,150), -1, 8, 0);
						 circle(colorin, mc[num_con[3]], 5, Scalar(0, 0,200), -1, 8, 0);
