...
private:
  ...
  void imageViewer()
  {
    ...
    cv::namedWindow("Image Viewercom");
    oss << "starting...";
    ...
    for(; running && ros::ok();)
    {
      if(updateImage)
      {
        ...
        cv::imshow("Image Viewercom", combined);
      }
    ...
    if(updateCloud) {
     ...
     //我自己的标定
      float fx = 534.6526;
				...
				float cy = 275.8264;
      ptMsg.point.z = pt.z;
      ptMsg.point.x = (img_x-cx)*pt.z/fx;
      ptMsg.point.y = (img_y-cy)*pt.z/fy;
      ...
      ossXYZ << "( " << ptMsg.point.x << ", " << ptMsg.point.y
                                  << ", " << ptMsg.point.z << " )";
      cv::putText(color, ossXYZ.str(), cv::Point(img_x, img_y), font, 0.5, colorText, 1, CV_AA);
  ...
  void dispDepth(const cv::Mat &in, cv::Mat &out, const float maxValue)
  {
    ...
    cv::applyColorMap(tmp, out, cv::COLORMAP_BONE);
  }
...
////////////
  void combine(const cv::Mat &inC, const cv::Mat &inD, cv::Mat &out)
  {
    ...
    for(int r = 0; r < inC.rows; ++r)
    {
      ...
      *itC = inC.ptr<cv::Vec3b>(r),
      ...
      {
        itO->val[0] = (itC->val[0] + ((itD->val[0]*40)%256))>> 1;
        itO->val[1] = (itC->val[1] + ((itD->val[1]*40)%256)) >> 1;
        itO->val[2] = (itC->val[2] + ((itD->val[2]*40)%256))>> 1;
      }
    }
  }
  ...
  void createCloud(const cv::Mat &depth, const cv::Mat &color, pcl::PointCloud<pcl::PointXYZRGBA>::Ptr &cloud) const
  {
    ...
    for(int r = 0; r < depth.rows; ++r)
    {
      ...
      const uint16_t *itD = depth.ptr<uint16_t>(r);//第r行头指针
