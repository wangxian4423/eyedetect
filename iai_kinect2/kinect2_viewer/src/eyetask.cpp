...
struct Object
{
	...
	}image_pose, world_pose;//image_pose指示图像坐标系坐标，world_pose指示世界坐标系坐标
 } object[7];
...
//
Mat pre_de=imread("//home//wxw//catkin_ws//0000_depth.png");
Mat pre_co=imread("//home//wxw//catkin_ws//0000_color.jpg");
...
//
      ...
      for(; running && ros::ok();)
          ...
          if(updateImage)
             ...
             depthcontrol(depth,depthshow,800,950);//深度图像筛选
...
void targetdetection(const cv::Mat &color,const cv::Mat &depth,const cv::Mat &combined,cv::Mat &target)
{
	...
	Mat depthin=depth;
	bool debug=0;
GaussianBlur(depth, depth, Size(3, 3), 0.1, 0, BORDER_DEFAULT);
 blur( depth, depth, Size(3,3) );  
 imshow("depthsrc",depth);
  	  ...
  	  else
  	  {
			...
			/*********检测4个大孔件的坐标信息*********/
					...
					if((contourArea(contours[i])>2000)&&(contourArea(contours[i])<2600))
					{
						 ...
						 if(1)//(0.8*rect.size.height<rect.size.width) && (rect.size.width<1.2*rect.size.height)
					  {
						count_hole++;
						switch(count_hole)
						{//先把孔件初略分类
						...
						if(depthin.at<uint16_t>(mc[i].y,mc[i].x)<850)
						{
							posehole=1;
						}
						else
				  ...
				  /************************立四棱柱****************************/
					...
					{
						 RotatedRect rect=minAreaRect(contours[i]);//画最小外接矩形
						 if((0.95*rect.size.width*rect.size.height)>contourArea(contours[i]) &&(0.5*rect.size.width*rect.size.height)<contourArea(contours[i]))
					  {
						mu[i] = moments(contours[i], false);
						 mc[i] = Point2d(mu[i].m10 / mu[i].m00, mu[i].m01 / mu[i].m00);//求中心点坐标
					...
					if((contourArea(contours[i])>150)&&(contourArea(contours[i])<240))
					{
						...
						if(depthin.at<uint16_t>(center.y,center.x)>840)
						{
					...
					if((contourArea(contours[i])>110)&&(contourArea(contours[i])<190))
					{
						...
						if(depthin.at<uint16_t>(center.y,center.x)<841) //
						{
					...
					if((contourArea(contours[i])>29)&&(contourArea(contours[i])<95))
					{
						mu[i] = moments(contours[i], false);
						 mc[i] = Point2d(mu[i].m10 / mu[i].m00, mu[i].m01 / mu[i].m00);//求中心点坐标
						if(depthin.at<uint16_t>(mc[i].y,mc[i].x)<842)
						{
						 if( 1)
					 		 {
...
//下面进行深度二值化，筛选出水平摆放的零件
 cv::Mat tmp2 = cv::Mat(depth.rows, depth.cols, CV_8U);
 for(int r = 0; r < depth.rows; ++r)
	  {
		  const uint16_t *itI2 = depth.ptr<uint16_t>(r);
		  uint8_t *itO2 = tmp2.ptr<uint8_t>(r);

		  for(int c = 0; c < depth.cols; ++c, ++itI2, ++itO2)
		  {
			  if((*itI2>855) && (*itI2<880))
			  {
				  *itO2 = (uint8_t)(*itI2-855);
			  }
